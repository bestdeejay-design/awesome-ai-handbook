...
6. [Пример: test-graphics — генерация картинок через сторонний API](#6-пример-test-graphics--генерация-картинок-через-сторонний-api)
7. [Прокачка скиллов: от простого к сложному](#7-прокачка-скиллов-от-простого-к-сложному)
8. [Скиллы этого хендбука](#8-скиллы-этого-хендбука)
...
### Что делает
- Генерирует плейсхолдеры (`placehold.co`) с заданными размерами и текстом
- Берёт рандомные фото (`loremflickr.com`) для реалистичных заглушек
- Создаёт иконки заданного размера
...
- Примеры: `.opencode/skills/data-science/`, `.opencode/skills/frontend/`
